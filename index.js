...
limitawal = 30
memberlimit = 0
ator = 'Unete y Suscribete'
namo = 'Soporte y Aportes Community'
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('qr already scan','white'))
})
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `@${num.split('@')[0]}\ Bienvenido Bro a *${mdata.subject}* Pasatela bien aqui`
				...
				teks = `eventualmente la carga del grupo se redujo en 1,Adios. @${num.split('@')[0]}`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
